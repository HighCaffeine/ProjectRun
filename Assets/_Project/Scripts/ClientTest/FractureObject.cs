...
public class FractureObject : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        sourceRenderer = skinnedRenderer != null && !HasStaticMesh()
            ? skinnedRenderer
    ...
    private void BreakInternal(Vector3 impactVelocity, Vector3 launchDirection, bool useDirection)
    {
        ...
        if (chunksRoot == null)
        {
            Debug.LogWarning($"{nameof(FractureObject)} on {name} could not create fracture chunks. Check MeshFilter or SkinnedMeshRenderer.", this);
            return;
        }
    ...
    private void PrepareFracture()
    {
        ...
        Mesh localMesh = ToLocalMesh(sourceMesh);
    ...
    private Mesh GetSourceMesh()
    {
        ...
        return bakedMesh.vertexCount > 0 ? bakedMesh : null;
    }
